...
odoo.define('pos_retail.model', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
            ...
            for (var i = 0; i < this.iot_boxes_save_orders.length; i++) {
                ...
                iot.rpc('/pos/ping/server', {
                    ...
                    if (!response_ping_odoo_server) {
                        self.set('synch', {
                            state: 'disconnected',
                            pending: _t('Odoo server down or network PosBox have problem, IoT could not ping to your Odoo with ip ' + self.config.posbox_save_orders_server_ip + ' and port:' + self.config.posbox_save_orders_server_port)
                        });
                    } else {
                ...
                iot.rpc('/pos/push/orders', {
                    ...
                    console.error('Error of /pos/push/orders: ')
                    console.error(error)
                })
            }
        ...
        show_popup_clients: function (keep_screen) {
            ...
            if (quickly_search_client) {
                ...
                this.gui.show_popup('popup_selection_extend', {
                    ...
                    sub_datas: this.db.get_partners_sorted(100),
        ...
        async auto_update_stock_products(product_list) {
            if (!product_list || product_list.length == 0 || !this.config.display_onhand) {
                return;
            }
            const product_ids = _.pluck(product_list, 'id');
            var location_ids = [];
            ...
            const picking_source = this.get_picking_source_location();
            ...
            for (let i = 0; i < product_ids.length; i++) {
                ...
                let $qty_available_product_element = $("article[data-product-id='" + product_id + "'] span[class='qty_available'] span[class='value']") || $("article[data-product-id='" + product_id + "'] span[class='qty_available'] span[class='value oe_hidden']");
                if (this.config.product_view == 'list') {
                    $qty_available_product_element = $("tr[data-product-id='" + product_id + "'] span[class='qty_available'] span[class='value']") || $("article[data-product-id='" + product_id + "'] span[class='qty_available'] span[class='value oe_hidden']");
                }
        ...
        _validate_by_manager: function (action_will_do_if_passing_security, title) {
            this.action_will_do_if_passing_security = action_will_do_if_passing_security;
            var self = this;
                        ...
                        return self.gui.show_popup('ask_password', {
                            ...
                            confirm: function (password) {
                                if (manager_user['pos_security_pin'] != password) {
                                    ...
                                    self.action_will_do_if_passing_security = null;
                                    setTimeout(function () {
        ...
        initialize: function (session, attributes) {
            ...
            payment_method_object.fields.push('is_customer_required');
            ...
            if (res_users_object) {
                ...
                var res_users_object_loaded = res_users_object.loaded;
                res_users_object.loaded = function (self, users) {
                    res_users_object_loaded(self, users);
                    self.barcode_by_user = {}
                    users.forEach(u => {
                        if (u.barcode) {
                            self.barcode_by_user[u.barcode] = u
                        }
                    })
                };
            }
        ...
        add_new_order: function () {
            ...
            self.trigger('update:sync_stock_production_lot');
        },
        ...
        _save_to_server: function (orders, options) {
            ...
            return _super_PosModel._save_to_server.call(this, orders, options).then(function (pos_order_backend_ids) {
                if (pos_order_backend_ids.length == 1) {
                    ...
                    self.trigger('request.sync.backend');
                    self.trigger('update:sync_stock_production_lot');
                }
        ...
        add_return_order: function (order_return, lines) {
            ...
            if (order_return['user_id']) {
                order['seller'] = order_return['user_id'][1]
            }
            this.get('orders').add(order);
            var pos_employee_id = order_return['pos_employee_id'];
            if (pos_employee_id) {
                var pos_employee_id = pos_employee_id[0];
                var pos_employee = this.employee_by_id[pos_employee_id];
                if (pos_employee) {
                    order['pos_employee_id'] = pos_employee.id;
                    order['employee'] = pos_employee;
                }
            }
    ...
    //     },
    //     async get_qty_on_hand(location_id) {
    //         this.qty_available = 0
    //         const stockValue = await rpc.query({
    //             model: 'stock.location',
    //             method: 'get_stock_data_by_location_ids',
    //             args: [[], [this.id], [location_id]],
    //             context: {}
    //         });
    //         if (stockValue && stockValue[location_id]) {
    //             this.qty_available = stockValue[location_id][this.id]
    //         }
