odoo.define('pos_retail.gui', function (require) {
    ...
    gui.Gui.include({
        ...
        show_saved_screen: function (order, options) { // todo: when open popup_login_viva_employee, not allow core close popup
            this._super(order, options);
            if (this.popup_current_display == 'popup_login_viva_employee') {
                this.show_popup('popup_login_viva_employee', {
                    title: 'Login with Employee',
                    body: 'Please choose one Employee and Input Your POS Pass Pin, POS Pass Pin setting at Employee/Point Of Sale'
                });
            }
        },
    });
