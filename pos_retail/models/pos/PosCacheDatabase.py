...
class PosCacheDatabase(models.Model):
    ...
    def get_modifiers_backend(self, write_date, res_model, config_id=None):
        ...
        if res_model != 'product.product':
            records = self.sudo().search([('write_date', '>', to_date), ('res_model', '=', res_model)], limit=100)
            ...
            product_template_ids = [p.id for p in self.env['product.template'].search([('write_date', '>', to_date)], limit=100)]
            products = self.env[res_model].search(
                ['|', ('write_date', '>', to_date), ('product_tmpl_id', 'in', product_template_ids)], limit=100)
    ...
    def install_data(self, model_name=None, min_id=0, max_id=1999):
        _logger.info('[install_data] install_data() model %s from id %s to id %s' % (model_name, min_id, max_id))
        self.env.cr.execute(
