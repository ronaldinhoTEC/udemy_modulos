...
odoo.define('pos_retail.order', function (require) {
    ...
    models.Order = models.Order.extend({
        ...
        init_from_JSON: function (json) {
            ...
            if (json.customer_ref) {
                this.set_picking_type(json.customer_ref)
            }
            return res;
        },
        export_as_JSON: function () {
            ...
            if (this.customer_ref) {
                json.customer_ref = this.customer_ref;
            }
            return json;
        },
        export_for_printing: function () {
            ...
            receipt['customer_ref'] = order.customer_ref;
        ...
        display_lot_popup: function () {
            ...
            if (this.pos.config.selection_existing_lots) {
                ...
                if (lotsExistingOfProduct === undefined) return;
        ...
        validate_order_return: function () {
            ...
            if (this.pos.config.required_reason_return) {
                var line_missed_input_return_reason = _.find(this.orderlines.models, function (line) {
                    return line.get_price_with_tax() < 0 && !line.has_input_return_reason() && !line.is_special;
                });
                if (line_missed_input_return_reason && this.pos.return_reasons && this.pos.return_reasons.length > 0) {
                    setTimeout(function () {
                        return self.pos.gui.show_popup('popup_selection_tags', {
                            selected_orderline: null,
        ...
        add_product: function (product, options) {
            ...
            let qty_available = this.pos.db.stock_datas[product.id];
            if (!this.pos.config.allow_order_out_of_stock && product.type === 'product' && (qty_available === null || qty_available === 0))
                return this.pos.gui.show_popup('confirm', {
                    title: _t('Warning'),
                    body: _t('El producto seleccionado no tiene stock, por favor, seleccione otro')
                });
        ...
        create_voucher: function (isGiftCard) {
            const self = this;
            let title = ''
            if (isGiftCard) {
                title = _t('Create Gift Card')
            } else {
                title = _t('Create Discount Card')
            }
            return this.pos._get_voucher_number().then(function (number) {
                self.pos.gui.show_popup('PopUpPrintVoucher', {
                    ...
                    confirm: function (voucher_val) {
                        voucher_val['description'] = voucher_val['description']
                        voucher_val['extra_description'] = voucher_val['extra_description']
                        ...
                        if (isGiftCard) {
                            voucher_val['card_type'] = 'gift'
                        } else {
                            voucher_val['card_type'] = 'discount'
                        }
                        return rpc.query({
                            model: 'pos.voucher',
    ...
    models.Orderline = models.Orderline.extend({
        ...
        init_from_JSON: function (json) {
            ...
            if (json.is_special) {
                this.is_special = json.is_special
            }
            return res;
        },
        export_as_JSON: function () {
            ...
            if (this.is_special) {
                json.is_special = this.is_special
            }
            return json;
        },
        clone: function () {
        ...
        set_unit_price: function (price) {
            ...
            // }
            // todo: kimanh removed feature minimum price
            _super_Orderline.set_unit_price.apply(this, arguments);
        },
        ...
        get_product_lot_names: function () {
            if (this.lot_ids && this.lot_ids.length) {
                return this.lot_ids.map(x => x['name']).join(' ');
            } else {
                return "-";
            }
        },
        has_input_return_reason: function () {
            if (this.tags && this.tags.length && !this.is_special) {
        ...
        can_be_merged_with: function (orderline) {
            ...
            return this.pos.config.can_be_merged;
        },
