...
odoo.define('pos_retail.screen_payment', function (require) {
    ...
    screens.PaymentScreenWidget.include({
        ...
        click_paymentmethods: function (id) {
            ...
            const payment_method = this.pos.payment_methods_by_id[id];
            const order = this.pos.get_order();
            if (payment_method && payment_method['is_customer_required'] && !order.get_client()) {
                this.pos.gui.show_screen('clientlist');
                return this.pos.gui.show_popup('dialog', {
                    title: payment_method['name'],
                    body: _t('Required Set Customer')
                })
            }
            if (payment_method && payment_method['is_customer_required'] && order && !order['customer_ref']) {
                self.hide()
                return self.gui.show_popup('textarea', {
                    title: _t('Please Set Customer Reference No.'),
                    value: '',
                    confirm: function (customer_ref) {
                        order.set_customer_ref(customer_ref);
                        self.click_paymentmethods(id)
                        self.show();
                        self.renderElement();
                    },
                    cancel: function () {
        ...
        renderElement: function () {
            ...
            this._super();
            this.$('.js_set_customer_ref').click(function () {
                self.hide();
                return self.gui.show_popup('textarea', {
                    title: _t('Please Set Customer Reference No.'),
