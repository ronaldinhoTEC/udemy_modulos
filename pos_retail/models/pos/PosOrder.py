...
class POSOrder(models.Model):
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        if context.get('pos_config_id', None):
            ...
            if config.load_orders_type == 'load_all':
                domain = domain
            if config.load_orders_type == 'last_1_days':
            ...
            if config.load_orders_type == 'last_3_days':
                ...
                domain.append(('create_date', '>=', loadFromDate))
            if config.load_orders_type == 'last_5_days':
    ...
    customer_ref = fields.Char('Customer Reference No.')
    ...
    def _order_fields(self, ui_order):
        if not ui_order.get('user_id', None):
            ui_order['user_id'] = self.env.user.id
        ...
        if ui_order.get('customer_ref', None):
            order_fields.update({
                'customer_ref': ui_order.get('customer_ref')
            })
        return order_fields
