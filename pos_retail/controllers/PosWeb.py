...
class pos_controller(PosController):
    ...
    def pos_web(self, config_id=False, **k):
        ...
        # session_info['stockDatas'] = []
        # if pos_session.config_id.display_onhand:
        #     location_ids = []
        #     if pos_session.config_id.multi_location and pos_session.config_id.stock_location_ids:
        #         for location in pos_session.config_id.stock_location_ids:
        #             if location.id not in location_ids:
        #                 location_ids.append(location.id)
        #     if pos_session.config_id.stock_location_id and pos_session.config_id.stock_location_id.id not in location_ids:
        #         location_ids.append(pos_session.config_id.stock_location_id.id)
        #     if len(location_ids) > 0:
        #         stockDatas = request.env['stock.location'].sudo().get_stock_data_by_location_ids([], location_ids)
        #     session_info['stockDatas'] = stockDatas
        session_info['start_at'] = fields.Datetime.to_string(pos_session.start_at)
        context = {
